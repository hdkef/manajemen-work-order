...
func (x *Entity) FindOneByID(db *sql.DB, ctx context.Context) error {
	return db.QueryRowContext(ctx, fmt.Sprintf("SELECT id,fullname,username,email,role,signature FROM %s WHERE id = ?", table.ENTITY), x.ID).Scan(&x.ID, &x.Fullname, &x.Username, &x.Email, &x.Role, &x.Signature)
}
...
func (x *Entity) FindAll(db *sql.DB, ctx context.Context) ([]Entity, error) {
	...
	rows, err := db.QueryContext(ctx, fmt.Sprintf("SELECT id,fullname,username,email,role,signature FROM %s", table.ENTITY))
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		var tmp Entity
		err = rows.Scan(&tmp.ID, &tmp.Fullname, &tmp.Username, &tmp.Email, &tmp.Role, &tmp.Signature)
		if err != nil {
			return nil, err
		}
