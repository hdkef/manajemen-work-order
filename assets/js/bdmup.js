...
let getBDMUPPPP = ()=>{
    hitEndpoint(`http://${API_HOST}/api/v1/bdmup/ppp`,null,"get").then((resJSON)=>{
...
let getBDMUPRP = ()=>{
    hitEndpoint(`http://${API_HOST}/api/v1/bdmup/rp`,null,"get").then((resJSON)=>{
...
let PPPAcc = (id,pppID)=>{
    hitEndpoint(`http://${API_HOST}/api/v1/ppp/${pppID}/ok/bdmup/${id}`,null,"post").then((resJSON)=>{
...
let RPAcc = (id,rpID)=>{
    hitEndpoint(`http://${API_HOST}/api/v1/rp/${rpID}/ok/bdmup/${id}`,null,"post").then((resJSON)=>{
        handleSuccess(resJSON)
    }).catch((err)=>{
        handleError(err)
    })
