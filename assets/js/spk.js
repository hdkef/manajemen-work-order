...
let LASTID = 0
let findLastID = ()=>{
    let tableBody = document.getElementById("table-spk")
...
let paging = ()=>{
    ...
    hitEndpoint(`http://${API_HOST}/api/v1/spk?last-id=${lastid}`,null,"get").then((resJSON)=>{
        populateTableSPK(resJSON)
    }).catch((err)=>{
        handleError(err)
    })
}

let populateTableSPK = (slice)=>{
    let tableSPK = document.getElementById("table-spk")
    for (let i=0;i<slice.length;i++){
        let id = +slice[i].id
        let dateCreated = slice[i].date_created
        let status = slice[i].status
        let workerEmail = slice[i].worker_email
        let doc = slice[i].doc
