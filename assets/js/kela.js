...
let getKELAPPP = ()=>{
    hitEndpoint(`http://${API_HOST}/api/v1/kela/ppp`,null,"get").then((resJSON)=>{
...
let getKELARP = ()=>{
    hitEndpoint(`http://${API_HOST}/api/v1/kela/rp`,null,"get").then((resJSON)=>{
...
let PPPAcc = (id,pppID)=>{
    hitEndpoint(`http://${API_HOST}/api/v1/ppp/${pppID}/ok/kela/${id}`,null,"post").then((resJSON)=>{
...
let RPAcc = (id,rpID)=>{
    hitEndpoint(`http://${API_HOST}/api/v1/rp/${rpID}/ok/kela/${id}`,null,"post").then((resJSON)=>{
        handleSuccess(resJSON)
    }).catch((err)=>{
        handleError(err)
    })
